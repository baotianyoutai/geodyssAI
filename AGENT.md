...
#### ■ GitHub 運用プロセス（Issue ＆ PR 駆動開発・教材化プロトコル）
本プロジェクトでは、オーナー自身の自習・完全再現を目的とした「教材化」を徹底するため、コード変更の前後で必ず以下のステップを踏む：
1. **GitHub Issue の作成（学習ガイド付き）**:
   - 各開発フェーズやスプリントのタスクに着手する前に、`github-mcp-server` を用いて GitHub 上に **Issue** を作成する。
   - Issue の本文には、そのタスクで**実行するコマンドや必要な公式ドキュメント（Webリンク）を網羅した「学習ガイド」**を記載する。
...
3. **Pull Request (PR) の作成と Issue 紐付け（解説付き）**:
   - 作業完了後、または作業の明確な区切りで GitHub 上に **Pull Request (PR)** を作成する。
   - PRの本文（説明欄）には、**実装したコードの説明、技術的な選択の理由（意図）、新しく学んだポイント（学習ポイント）、および参考にした公式Docsへのリンク（3点セット）**を含める。
   - PRは該当する Issue に紐付ける（例: `Closes #1`）。
   - **マージ権限はオーナー（Yuta）のみ**とし、エージェントは PR 作成までを行い、マージはオーナーのコードレビュー後に実行される。
4. **CHANGELOG.md への追記**:
