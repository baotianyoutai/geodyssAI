  ...
  return (
    <group>
      <points ref={pointsRef1}>
        ...
        <pointsMaterial size={0.06} color="#EDF2FB" transparent opacity={0.3} />
      </points>
      <points ref={pointsRef2}>
        <bufferGeometry>
          <bufferAttribute attach="attributes-position" args={[positions2, 3]} />
        </bufferGeometry>
        <pointsMaterial size={0.10} color="#8aebff" transparent opacity={0.5} />
      </points>
      <points ref={pointsRef3}>
        <bufferGeometry>
          <bufferAttribute attach="attributes-position" args={[positions3, 3]} />
        </bufferGeometry>
        <pointsMaterial size={0.14} color="#d0bcff" transparent opacity={0.6} />
      </points>
    </group>
  );
}
...
function SceneSetup({ onDepthChange, selectedStar, transitioning, onTransitionComplete, controlsRef }: SceneSetupProps) {
  ...
  useEffect(() => {
    scene.fog = new THREE.FogExp2('#050B18', 0.028);
      ...
      <div className="absolute bottom-8 left-8 right-8 flex justify-between items-end z-10 pointer-events-none">
        ...
        <div className="bg-slate-950/70 backdrop-blur-md border border-slate-800/80 rounded-xl p-4 shadow-2xl flex flex-col gap-2 pointer-events-auto">
          ...
          <div className="flex items-center gap-3">
            ...
            <span className="font-mono text-xs text-slate-200">Azure (Surface) - 表層 [強光発光]</span>
          </div>
          <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-3">
            <div className="w-2 h-2 rounded-full bg-slate-700 opacity-30"></div>
            <span className="font-mono text-xs text-slate-500">Midnight (Deep) - 深層 [漆黒減光]</span>
          </div>
        </div>
