...
以下の JSON フォーマットのみで返答してください（余計な解説テキストやコードブロック記号は含めないでください）:
{
  ...
  "comment": "この記事の核心をマンチカン航海士の口調（語尾〜ニャ）でまとめた短評コメント（2文程度）"
        ...
        let resultData: TldrData | null = null;
        ...
        if (resultData) {
          ...
          setTldr({
            points: [
              `${article.title} の主要概念と最新実装アプローチ解説`,
              `カテゴリ「${article.category || 'GenAI'}」における具体的な構成手順`,
              `実践的な環境構築およびコードサンプルの提示`
            ],
            comment: `この記事は ${article.title} について分かりやすく解説されたおすすめの技術星だニャ！しっかり読んで知識を深めてほしいニャ 🐾`
          });
        }
      } catch (err) {
        console.warn('TL;DR generation error:', err);
      } finally {
  ...
  // 4. AI 機能 2: 3ステップ深掘り学習（Google Skills, Kaggle, 公式Docs, 論文等の自動引用・推薦）
  useEffect(() => {
...
以下の JSON フォーマットのみで返答してください（余計な解説テキストは含めないでください）:
{
  "handsOn": {
  ...
  "advancedResearch": {
    ...
    "platform": "arXiv 論文 / Google Research / Medium"
        ...
        const CANDIDATES = ['gemini-3.5-flash', 'gemini-2.5-flash'];
        let stepupData: StepupLearningData | null = null;
        ...
        console.warn('Step-up learning generation error:', err);
      } finally {
...
3. 2〜4文程度でコンパクトに分かりやすく答えてください。`;
      ...
      if (!answerText) {
        answerText = `ご質問「${userText}」についてニャ！この記事のポイントは「${article.title}」の実装と概念にあるニャ。より詳しいコード検証は手元で試してみるのが一番だニャ 🐾`;
      }
      ...
      setMessages(prev => [...prev, {
        sender: 'bot',
        text: '通信状況により応答が遅れて申し訳ないニャ！記事の要点をもう一度見直してみてほしいニャ 🐾'
      }]);
    } finally {
      setAnswering(false);
    }
